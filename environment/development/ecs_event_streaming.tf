/**
 * ECS Event Streaming
 * This component enables access to the ECS event logs for your cluster by creating a CloudWatch event rule that
 * listens for events from the cluster and triggers a Lambda function to write the events to CloudWatch logs.
 * The component also creates a CloudWatch dashboard that presents the results of a logs insights query run on
 * the Lambda logs.
 */

# Cloudwatch event rule to trigger the lambda function
resource "aws_cloudwatch_event_rule" "ecs_event_stream" {
  name        = "${var.application}-${var.environment}-ecs-event-stream"
  description = "Passes ecs event logs for ${var.application}-${var.environment} to a lambda function that writes them to cw logs"

  event_pattern = <<PATTERN
  {
    "detail": {
      "clusterArn": ["${aws_ecs_cluster.application.arn}"]
    }
  }
...
data "archive_file" "lambda_zip" {
  ...
  output_path             = "lambda-${var.application}.zip"
}
...
resource "aws_lambda_function" "ecs_event_stream" {
  function_name    = "${var.application}-${var.environment}-ecs-event-stream"
...
# CloudWatch dashboard that includes a logs insights query for analyzing log data.
resource "aws_cloudwatch_dashboard" "ecs-event-stream" {
  dashboard_name = "${var.application}-${var.environment}-ecs-event-stream"
  ...
  "widgets": [
    {
      ...
      "height": 18,
      "properties": {
        "query": "SOURCE '/aws/lambda/${var.application}-${var.environment}-ecs-event-stream' | fields @timestamp as time, detail.desiredStatus as desired, detail.lastStatus as latest, detail.stoppedReason as reason, detail.containers.0.reason as container_reason, detail.taskDefinitionArn as task_definition\n| filter @type != \"START\" and @type != \"END\" and @type != \"REPORT\"\n| sort detail.updatedAt desc, detail.version desc\n| limit 100",
        "region": "${var.region}",
        "title": "ECS Event Log"
      }
    }
