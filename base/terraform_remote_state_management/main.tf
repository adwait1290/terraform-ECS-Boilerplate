/**
 * A Terraform module that configures an s3 bucket for use with Terraform's remote terraform_remote_state_management feature.
 *
 * Useful for creating a common bucket naming convention and attaching a bucket policy using the specified role.
 */

# the primary role that will be used to access the tf remote terraform_remote_state_management
variable "role" {
}
...
# the application that will be using this remote terraform_remote_state_management
...
# if enabled, we will create a dynamodb table that can be used to store terraform_remote_state_management file lock status
variable "dynamodb_state_locking" {
  default = false
}

# bucket for storing tf terraform_remote_state_management
...
# dynamodb table used to storing lock terraform_remote_state_management
